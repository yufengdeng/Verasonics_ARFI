...
scriptName='SETUPC5_2Shear_wave_STL';
push_focus = 50; %mm
push_Fnum = 2; % 
pushCycle  = 800;
ne = 50;
nrefs = 5;
npush = 6; % number of pushes

%% Define basic parameters
m = 64; % Bmode lines
bmode_focus = 50;
bmode_Fnum = 2;
pushAngleDegree = linspace(10,2.5,npush);
...
PData(2).Origin(1,1) = 0; 
PData(2).Origin(1,2) = 0;
PData(2).Origin(1,3) = ceil(radius * cos(-scanangle/4)) - radius - 5;

PData(2).Region = struct(...
    'Shape',struct('Name','Sector','Position',[0,0,-radius],'r1',radius+P.startDepth,'r2',radius+P.endDepth,'angle',0.002));
...
Resource.RcvBuffer(2).numFrames = npush;

Resource.VDAS.dmaTimeout = 8000;

Resource.InterBuffer(1).datatype = 'complex';
Resource.InterBuffer(1).numFrames = npush;
...
TX(n).Apod = [zeros(1,34) ones(1,60) zeros(1,34)]; %F/2 at 50 mm
TX(n).Delay = computeTXDelays(TX(n));
...
elementAngleDegree = Trans.ElementPos(:,4)/pi*180;
for ipush = 1:npush
    n = m+1+ipush;
    TX(n).waveform = 2;
...
Receive = repmat(struct('Apod', zeros(1,Trans.numelements), ...
                        ...
                        'sampleMode','NS200BW', ...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, Resource.RcvBuffer(1).numFrames*m+ne*npush);
